...
* In the line listing the template, click on the ‘metadata’ tag (![](https://github.com/metadatacenter/cedar-manual/raw/master/docs/assets/imgs/IntelligentAuthoringGuideMetadataTag.png)). This will generate a form to fill out the metadata. Or, try this with the 'metadata' tag ![metadata icon](https://github.com/metadatacenter/cedar-manual/raw/master/docs/assets/imgs/IntelligentAuthoringGuideMetadataTag.png){:height="16px"} to see how that works.  Alternatively, you can select your template and click on the template options menu (displayed as three vertical dots), choosing “Populate” to start entering metadata for the template.
